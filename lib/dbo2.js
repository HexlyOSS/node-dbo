...
class DboError extends Error {
  constructor(message, cause, path, operation, stack) {
    super(message)
...
const defaultConverter = ({ yesql, dbo } ) => (sql, path) => {
  const prepared = yesql(sql)

  const onError = (trace, operation) => cause => {
    throw new DboError(cause.message, cause, path, operation, trace.stack)
  }

  const root = async (params) => {
    const trace = {}
    Error.captureStackTrace(trace)
    const { text, values } = prepared(params)
    return dbo.task(path, t => t.query(text, values).catch(onError(trace)) )
  }

  ['one', 'many', 'any', 'none', 'manyOrNone'].forEach( operation => {
    root[operation] = (params) => {
      const trace = {}
      Error.captureStackTrace(trace)
      const { text, values } = prepared(params)
      return dbo.task(path, t => t[operation](text, values).catch(onError(trace, operation)) )
    }
  })
...
const initDbo = async (cfg={}) => {
  ...
  config.pgConfig = {
    ...
    extend(obj, dc){
      const compiled = recurseMap(templates, defaultConverter({ yesql, dbo: obj, pgp }))
