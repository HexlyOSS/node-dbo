...
class DboError extends Error {
  constructor(message, cause, path, operation, stack, trace) {
    super(message)
    ...
    if( process.env.DBO_INCLUDE_PARAMS ){

    }else{
      // TODO sanitize smart?
      Object.keys(this.trace.params).forEach(p => {
        const val = this.trace.params[p]
        let type = typeof(val)
        ...
        this.trace.params[p] =  `*REDACTED <${type}>*`
      })
    }
...
const defaultConverter = ({ yesql, dbo } ) => (query, path) => {
  ...
  const preparedStatement = yesql(parsedQuery)
  ...
  const root = async (parameters) => {
    ...
    const { text: sql, values: params } = preparedStatement(parameters)
